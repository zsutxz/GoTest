...
func CreateAttendanceRecord(c *gin.Context, dto *model.AttendanceRecordCreateDTO) error {
	var total int64
	conf.HrmsDB(c).Model(&model.AttendanceRecord{}).Where("staff_id = ? and date = ?", dto.StaffId, dto.Date).Count(&total)
	if total != 0 {
		return errors.New(fmt.Sprintf("该月考勤数据已经存在"))
	}
	var attendanceRecord model.AttendanceRecord
	util.Transfer(&dto, &attendanceRecord)
	attendanceRecord.AttendanceId = RandomID("attendance_record")
	if err := conf.HrmsDB(c).Create(&attendanceRecord).Error; err != nil {
...
func DelAttendRecordByAttendId(c *gin.Context, attendanceId string) error {
	if err := conf.HrmsDB(c).Where("attendance_id = ?", attendanceId).Delete(&model.AttendanceRecord{}).
		Error; err != nil {
...
func UpdateAttendRecordById(c *gin.Context, dto *model.AttendanceRecordEditDTO) error {
	...
	util.Transfer(&dto, &attentRecord)
	if err := conf.HrmsDB(c).Model(&model.AttendanceRecord{}).Where("id = ?", attentRecord.ID).
...
func GetAttendRecordByStaffId(c *gin.Context, staffId string, start int, limit int) ([]*model.AttendanceRecord, int64, error) {
	...
	if start == -1 && limit == -1 {
		// 不加分页
		if staffId != "all" {
			err = conf.HrmsDB(c).Where("staff_id = ?", staffId).Order("date desc").Find(&records).Error
		} else {
			err = conf.HrmsDB(c).Order("date desc").Find(&records).Error
		}

	} else {
		// 加分页
		if staffId != "all" {
			err = conf.HrmsDB(c).Where("staff_id = ?", staffId).Offset(start).Limit(limit).Order("date desc").Find(&records).Error
		} else {
			err = conf.HrmsDB(c).Offset(start).Limit(limit).Order("date desc").Find(&records).Error
		}
	}
	if err != nil {
		return nil, 0, err
	...
	var total int64
	conf.HrmsDB(c).Model(&model.AttendanceRecord{}).Count(&total)
	if staffId != "all" {
...
and attend.date = salary.salary_date where salary.is_pay = 2 order by attend.date desc`
	if start == -1 && limit == -1 {
		// 不加分页
		if staffId != "all" {
			err = conf.HrmsDB(c).Raw(sqlReq1, staffId).Find(&records).Error
		} else {
			err = conf.HrmsDB(c).Raw(sqlReq2).Find(&records).Error
		}

	} else {
		// 加分页
		if staffId != "all" {
			err = conf.HrmsDB(c).Raw(sqlReq1, staffId).Offset(start).Limit(limit).Find(&records).Error
		} else {
			err = conf.HrmsDB(c).Raw(sqlReq2).Offset(start).Limit(limit).Find(&records).Error
		}
	}
	if err != nil {
		return nil, 0, err
...
func GetAttendRecordIsPayByStaffIdAndDate(c *gin.Context, staffId string, date string) bool {
	var total int64
	conf.HrmsDB(c).Model(&model.SalaryRecord{}).Where("staff_id = ? and salary_date = ? and is_pay = 2", staffId, date).Count(&total)
	return total != 0
}
...
func GetAttendRecordApproveByLeaderStaffId(c *gin.Context, leaderStaffId string) ([]*model.AttendanceRecord, int64, error) {
	...
	conf.HrmsDB(c).Where("leader_staff_id = ?", leaderStaffId).Find(&staffs)
	if len(staffs) == 0 {
		return nil, 0, nil
	}
	// 查询下属是否有未审批的考勤申请
	...
	for _, staff := range staffs {
		...
		conf.HrmsDB(c).Where("staff_id = ? and approve = 0", staffId).Find(&attend)
		if attend != nil {
...
func Compute(c *gin.Context, attendId string) error {
	err := conf.HrmsDB(c).Transaction(func(tx *gorm.DB) error {
		// 更新考勤信息为审批通过状态
