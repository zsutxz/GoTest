...
func DepartCreate(c *gin.Context) {
	...
	conf.HrmsDB(c).Where("dep_name = ?", departmentCreateDTO.DepName).First(&departmentCheck)
	...
	if result = conf.HrmsDB(c).Create(&departmentCreate); result.Error != nil {
		result.Rollback()
	...
	if result = conf.HrmsDB(c).Where("id = ?", departmentCreate.ID); result.Error != nil {
...
func DepartEdit(c *gin.Context) {
	...
	conf.HrmsDB(c).Model(&model.Department{}).Where("dep_id = ?", departmentEditDTO.DepId).
...
func DepartQuery(c *gin.Context) {
	...
	if depId == "all" {
		// 查询全部
		if start == -1 && start == -1 {
			conf.HrmsDB(c).Find(&deps)
		} else {
			conf.HrmsDB(c).Offset(start).Limit(limit).Find(&deps)
		}
		if len(deps) == 0 {
			// 不存在
			code = 2001
		...
		// 总记录数
		conf.HrmsDB(c).Model(&model.Department{}).Count(&total)
		c.JSON(200, gin.H{
			"status": code,
			"total":  total,
			"msg":    deps,
	...
	conf.HrmsDB(c).Where("dep_id = ?", depId).Find(&deps)
	if len(deps) == 0 {
		// 不存在
		code = 2001
	}
...
func DepartDel(c *gin.Context) {
	depId := c.Param("dep_id")
	if err := conf.HrmsDB(c).Where("dep_id = ?", depId).Delete(&model.Department{}).Error; err != nil {
