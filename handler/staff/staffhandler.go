...
func StaffCreate(c *gin.Context) {
	...
	var exist int64
	conf.HrmsDB(c).Model(&model.Staff{}).Where("identity_num = ? or staff_id = ?", staffCreateDto.IdentityNum, staffId).Count(&exist)
	if exist != 0 {
	...
	conf.HrmsDB(c).Where("staff_id = ?", staffCreateDto.LeaderStaffId).Find(&leader)
...
func StaffEdit(c *gin.Context) {
	...
	// 查询leader名称
	var leader model.Staff
	conf.HrmsDB(c).Where("staff_id = ?", staffEditDTO.LeaderStaffId).Find(&leader)
	staff.LeaderName = leader.StaffName
	conf.HrmsDB(c).Model(&model.Staff{}).Where("staff_id = ?", staffEditDTO.StaffId).
		Updates(&staff)
	c.JSON(200, gin.H{
		"status": 2000,
	})
...
func StaffQuery(c *gin.Context) {
	...
	if staffId == "all" {
		// 查询全部
		if start == -1 && start == -1 {
			conf.HrmsDB(c).Where("staff_id != 'root' and staff_id != 'admin'").Find(&staffs)
		} else {
			conf.HrmsDB(c).Where("staff_id != 'root' and staff_id != 'admin'").Offset(start).Limit(limit).Find(&staffs)
		}
		if len(staffs) == 0 {
			// 不存在
			code = 2001
		...
		// 总记录数
		conf.HrmsDB(c).Model(&model.Staff{}).Where("staff_id != 'root' and staff_id != 'admin'").Count(&total)
		c.JSON(http.StatusOK, gin.H{
			"status": code,
			"total":  total,
	...
	conf.HrmsDB(c).Where("staff_id = ? and staff_id != 'root' and staff_id != 'admin'", staffId).Find(&staffs)
	if len(staffs) == 0 {
		// 不存在
		code = 2001
	}
...
func getRuleByStaffId(c *gin.Context, staffId string) string {
	...
	if err := conf.HrmsDB(c).Where("staff_id = ?", staffId).Find(&authority).Error; err == nil {
...
func StaffQueryByName(c *gin.Context) {
	...
	conf.HrmsDB(c).Where("staff_name like ?", "%"+staffName+"%").Where("staff_id != 'root' and staff_id != 'admin'").Find(&staffs)
	if len(staffs) == 0 {
		// 不存在
		code = 2001
	}
...
func StaffQueryByDep(c *gin.Context) {
	...
	conf.HrmsDB(c).Raw(reqSql).Scan(&staffs)
	if len(staffs) == 0 {
		// 不存在
		code = 2001
	}
...
func StaffDel(c *gin.Context) {
	staffId := c.Param("staff_id")
	if err := conf.HrmsDB(c).Where("staff_id = ?", staffId).Delete(&model.Staff{}).Error; err != nil {
	...
	// 密码删除
	if err := conf.HrmsDB(c).Where("staff_id = ?", staffId).Delete(&model.Authority{}).Error; err != nil {
		log.Printf("[StaffDel] err = %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{
			"status": 5001,
			"msg":    err,
