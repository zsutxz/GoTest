...
func CreateExample(c *gin.Context, dto *model.ExampleCreateDTO) error {
	...
	util.Transfer(&dto, &example)
	example.ExampleId = RandomID("example")
	if err := conf.HrmsDB(c).Create(&example).Error; err != nil {
...
func DelExampleByExampleId(c *gin.Context, example_id string) error {
	if err := conf.HrmsDB(c).Where("example_id = ?", example_id).Delete(&model.Example{}).Error; err != nil {
...
func UpdateExampleById(c *gin.Context, dto *model.ExampleEditDTO) error {
	var example model.Example
	util.Transfer(&dto, &example)
	if err := conf.HrmsDB(c).Where("id = ?", example.ID).
...
func GetExampleByName(c *gin.Context, name string, start int, limit int) ([]*model.Example, int64, error) {
	...
	if start == -1 && limit == -1 {
		// 不加分页
		if name != "all" {
			err = conf.HrmsDB(c).Where("name like ?", "%"+name+"%").Order("date desc").Find(&records).Error
		} else {
		...
		// 加分页
		if name != "all" {
			err = conf.HrmsDB(c).Where("name like ?", "%"+name+"%").Order("date desc").Offset(start).Limit(limit).Find(&records).Error
		} else {
			err = conf.HrmsDB(c).Order("date desc").Offset(start).Limit(limit).Find(&records).Error
		}
	}
	if err != nil {
		return nil, 0, err
	...
	var total int64
	conf.HrmsDB(c).Model(&model.Example{}).Count(&total)
	if name != "all" {
...
func RenderExample(c *gin.Context, id int64) (map[string]interface{}, error) {
	...
	if err = conf.HrmsDB(c).Where("id = ?", id).Find(&examples).Error; err != nil {
...
func CreateExampleScore(c *gin.Context, dto *model.ExampleScoreCreateDTO) (int64, error) {
	...
	util.Transfer(&dto, &exampleScore)
	// 判定成绩
	exampleScore.Score = getScore(exampleScore.Content, exampleScore.Commit)
	if err := conf.HrmsDB(c).Create(&exampleScore).Error; err != nil {
...
func GetExampleHistoryByName(c *gin.Context, name string, start int, limit int) ([]*model.ExampleScore, int64, error) {
	...
	var total int64
	conf.HrmsDB(c).Model(&model.ExampleScore{}).Count(&total)
	if name != "all" {
		total = int64(len(records))
	}
	return records, total, nil
...
func GetExampleHistoryByStaffId(c *gin.Context, staffId string, start int, limit int) ([]*model.ExampleScore, int64, error) {
	...
	if start == -1 && limit == -1 {
		...
		// 加分页
		if staffId != "all" {
			err = conf.HrmsDB(c).Where("staff_id = ?", staffId).Order("date desc").Offset(start).Limit(limit).Find(&records).Error
		} else {
			err = conf.HrmsDB(c).Order("date desc").Offset(start).Limit(limit).Find(&records).Error
		}
	}
