...
func CreateCandidate(c *gin.Context, dto *model.CandidateCreateDTO) error {
	...
	util.Transfer(&dto, &candidateRecord)
	candidateRecord.CandidateId = RandomID("candidate")
	if err := conf.HrmsDB(c).Create(&candidateRecord).Error; err != nil {
...
func DelCandidateByCandidateId(c *gin.Context, candidateId string) error {
	if err := conf.HrmsDB(c).Where("candidate_id = ?", candidateId).Delete(&model.Candidate{}).
		Error; err != nil {
...
func UpdateCandidateById(c *gin.Context, dto *model.CandidateEditDTO) error {
	...
	util.Transfer(&dto, &candidate)
	if err := conf.HrmsDB(c).Model(&model.Candidate{}).Where("id = ?", candidate.ID).
...
func GetCandidateByName(c *gin.Context, name string, start int, limit int) ([]*model.Candidate, int64, error) {
	...
	if start == -1 && limit == -1 {
		// 不加分页
		if name != "all" {
			err = conf.HrmsDB(c).Where("name like ?", "%"+name+"%").Find(&records).Error
		} else {
		...
		// 加分页
		if name != "all" {
			err = conf.HrmsDB(c).Where("name like ?", "%"+name+"%").Offset(start).Limit(limit).Find(&records).Error
		} else {
	...
	var total int64
	conf.HrmsDB(c).Model(&model.Candidate{}).Count(&total)
	if name != "all" {
...
func GetCandidateByStaffId(c *gin.Context, staffId string, start int, limit int) ([]*model.Candidate, int64, error) {
	...
	if start == -1 && limit == -1 {
		// 不加分页
		if staffId != "all" {
			err = conf.HrmsDB(c).Where("staff_id = ?", staffId).Find(&records).Error
		} else {
			err = conf.HrmsDB(c).Find(&records).Error
		}
		...
		// 加分页
		if staffId != "all" {
			err = conf.HrmsDB(c).Where("staff_id = ?", staffId).Offset(start).Limit(limit).Find(&records).Error
		} else {
			err = conf.HrmsDB(c).Offset(start).Limit(limit).Find(&records).Error
		}
	}
...
func SetCandidateRejectById(c *gin.Context, id int64) error {
	if err := conf.HrmsDB(c).Where("id = ?", id).
