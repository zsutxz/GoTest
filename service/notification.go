...
func GetNotificationByTitle(c *gin.Context, noticeTitle string, start int, limit int) ([]*model.Notification, int64, error) {
	...
	if start == -1 && limit == -1 {
		// 不加分页
		if noticeTitle != "all" {
			err = conf.HrmsDB(c).Where("notice_title like ?", "%"+noticeTitle+"%").Order("date desc").Find(&notifications).Error
		} else {
			err = conf.HrmsDB(c).Order("date desc").Find(&notifications).Error
		}

	} else {
		// 加分页
		if noticeTitle != "all" {
			err = conf.HrmsDB(c).Where("notice_title like ?", "%"+noticeTitle+"%").Order("date desc").Offset(start).Limit(limit).Find(&notifications).Error
		} else {
			err = conf.HrmsDB(c).Order("date desc").Offset(start).Limit(limit).Find(&notifications).Error
		}
	}
	if err != nil {
		return nil, 0, err
	...
	var total int64
	conf.HrmsDB(c).Model(&model.Notification{}).Count(&total)
	if noticeTitle != "all" {
...
func CreateNotification(c *gin.Context, dto *model.NotificationDTO) error {
	...
	util.Transfer(&dto, &notification)
	...
	if err := conf.HrmsDB(c).Create(&notification).Error; err != nil {
	...
	if notification.Type == "紧急通知" {
		...
		if err := conf.HrmsDB(c).Find(&staffs).Error; err != nil {
			log.Printf("CreateNotification err = %v", err)
			return err
		}
		// 获取员工手机号，发送紧急通知短信
...
func DelNotificationById(c *gin.Context, notice_id string) error {
	if err := conf.HrmsDB(c).Where("notice_id = ?", notice_id).Delete(&model.Notification{}).Error; err != nil {
...
func UpdateNotificationById(c *gin.Context, dto *model.NotificationEditDTO) error {
	...
	util.Transfer(&dto, &notification)
	notification.Date = Str2Time(dto.Date, 0)
	if err := conf.HrmsDB(c).Where("id = ?", notification.ID).
