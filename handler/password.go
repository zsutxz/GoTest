...
func PasswordQuery(c *gin.Context) {
	...
	// 总记录数
	conf.HrmsDB(c).Where("staff_id != 'root' and staff_id != 'admin'").Model(&model.Staff{}).Count(&total)
	psws = result
...
func buildPasswordQueryResult(c *gin.Context, staffId string, start int, limit int) ([]model.PasswordQueryVO, error) {
	...
	if staffId == "all" {
		...
		if start == -1 && limit == -1 {
			// 不加分页
			err = conf.HrmsDB(c).Where("staff_id != 'root' and staff_id != 'admin'").Find(&loginList).Error
		} else {
			// 加分页
			err = conf.HrmsDB(c).Where("staff_id != 'root' and staff_id != 'admin'").Offset(start).Limit(limit).Find(&loginList).Error
		}
	} else {
		// 查询单个用户
		err = conf.HrmsDB(c).Where("staff_id != 'root' and staff_id != 'admin'").Where("staff_id = ?", staffId).First(&loginList).Error
	}
	if err != nil {
...
func convertStaffIdToName(c *gin.Context, staffId string) string {
	var result Result
	conf.HrmsDB(c).Raw("select staff_name from staff where staff_id = ?", staffId).Scan(&result)
...
func PasswordEdit(c *gin.Context) {
	...
	if err := conf.HrmsDB(c).Where("staff_id = ?", staffId).Updates(&model.Authority{
